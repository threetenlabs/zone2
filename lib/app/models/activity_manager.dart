...
class HealthActivityManager extends GetxController {
  ...
  final _heartRateRecords = RxList<HeartRateRecord>([]);
  final _calorieRecords = RxList<CalorieBurnedRecord>([]);
  final _hourlyCalorieRecords = RxList<CalorieBurnedRecord>([]);
  final _stepRecords = RxList<StepRecord>([]);
  final _workoutRecords = RxList<WorkoutRecord>([]);
  final _buckets = RxList<HealthDataBucket>([]);

  // Convert statistics to Rx
  final _totalSteps = 0.obs;
  final _totalCaloriesBurned = 0.0.obs;
  final _totalZonePoints = 0.obs;
  final _multipleCalorieSources = false.obs;

  // Convert zone minutes to RxMap
  final _zoneMinutes = RxMap<int, int>({
    1: 0, // Very Light
    2: 0, // Light
    3: 0, // Moderate
    4: 0, // Hard
  ...
  final Map<int, ZoneConfig> _zoneConfigs = {
    1: const ZoneConfig(
  ...
  void processActivityData({
    ...
    // Parse records
    _heartRateRecords.value = _parseHeartRateData(
        activityData.where((data) => data.type == HealthDataType.HEART_RATE).toList());
    _calorieRecords.value = parseCalorieData(
    ...
    _workoutRecords.value = _parseWorkoutData(
        activityData.where((data) => data.type == HealthDataType.WORKOUT).toList());

    _multipleCalorieSources.value = _calorieRecords.map((record) => record.sourceName).toSet().length > 1;
  ...
  void _processHeartRateZones(int userAge) {
    ...
    _totalZonePoints.value = _zoneMinutes.entries.fold(0, (sum, entry) {
  ...
  void _processCaloriesByHour() {
    ...
    _hourlyCalorieRecords.value = hourlyTotals.entries.map((entry) {
  ...
  void _calculateTotals() {
    ...
    _totalSteps.value = _stepRecords.fold(0, (sum, record) => sum + record.numericValue.toInt());

    // Add steps from workouts
    _totalSteps.value += _workoutRecords.fold(0, (sum, record) => sum + record.totalSteps);

    // Track processed time ranges to avoid double counting
    Set<String> processedRanges = {};

    // Only use calories from regular records, with deduplication
    _totalCaloriesBurned.value = _calorieRecords.fold(0.0, (sum, record) {
      String timeRange = '${record.dateFrom}-${record.dateTo}';
  ...
  void _resetAllValues() {
    _heartRateRecords.clear();
    _calorieRecords.clear();
    _stepRecords.clear();
    _workoutRecords.clear();
    _buckets.clear();
    _totalSteps.value = 0;
    _totalCaloriesBurned.value = 0.0;
    _zoneMinutes.updateAll((key, value) => 0);
  }
  ...
  List<CalorieBurnedRecord> get calorieRecords => _calorieRecords;
  List<StepRecord> get stepRecords => _stepRecords;
  List<WorkoutRecord> get workoutRecords => _workoutRecords;
  List<HealthDataBucket> get buckets => _buckets;
  ...
  List<CalorieBurnedRecord> get hourlyCalorieRecords => _hourlyCalorieRecords;
}
