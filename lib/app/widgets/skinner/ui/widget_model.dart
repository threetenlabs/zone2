...
class OBJHome extends StatelessWidget {
  ...
  Widget build(context) {
    return Container(
      color: const Color(0xFF223388),
      child: const OBJViewer(),
    );
  }
}
...
class OBJViewerState extends State<OBJViewer> {
  ...
  void _handleDragUpdate(DragUpdateDetails dragUpdateDetails) {
    _rotation *=
        vec32.Quaternion.axisAngle(vec32.Vector3(0.0, 1.0, 0.0), dragUpdateDetails.delta.dx / 100);
    _rotation *=
        vec32.Quaternion.axisAngle(vec32.Vector3(1.0, 0.0, 0.0), dragUpdateDetails.delta.dy / 100);
    _rotation.normalize();
    _updateTransform();
  }
  ...
  void _updateTransform() {
    final modelMatrix =
        vec32.Matrix4.compose(vec32.Vector3.zero(), _rotation, vec32.Vector3.all(1.0));

    final viewMatrix = vec32.makeViewMatrix(
        vec32.Vector3(0.0, 0.0, 4.0), vec32.Vector3(0.0, 0.0, 0.0), vec32.Vector3(0.0, 1.0, 0.0));
  ...
  void _parseOBJFile() {
    ...
    for (var line in objLines) {
      ...
      if (line.startsWith('v ')) {
        ...
        positions.add(
            vec32.Vector3(double.parse(args[1]), double.parse(args[2]), double.parse(args[3])));
  ...
  Future<void> _loadMTLTextures() async {
    ...
    for (var mtl in _materials.values) {
      if (mtl.texturePath != null) {
        debugPrint('loading texture: ${mtl.texturePath}');
        ...
        AssetImage(path.join(_basePath, mtl.texturePath), bundle: _bundle)
          ...
          ImageStreamListener((ImageInfo info, bool _) {
            debugPrint('loaded texture: ${mtl.texturePath}');
...
class VertexMesh {
  ...
  void log() {
    for (int i = 0; i < indices.length; i += 3) {
      ...
      debugPrint(
          'f: {${x0.toStringAsFixed(3)}, ${y0.toStringAsFixed(3)}, ${z0.toStringAsFixed(3)}}, {${x1.toStringAsFixed(3)}, ${y1.toStringAsFixed(3)}, ${z1.toStringAsFixed(3)}}, {${x2.toStringAsFixed(3)}, ${y2.toStringAsFixed(3)}, ${z2.toStringAsFixed(3)}}');
    }
  }
}
  ...
  void _cacheVertices() {
    ...
    for (int i = 0; i < _mesh.vertexCount; ++i) {
      vec32.Vector4 position = vec32.Vector4(
          _mesh.positions[i * 3 + 0], _mesh.positions[i * 3 + 1], _mesh.positions[i * 3 + 2], 1.0);
    ...
    // Depth sort
    {
      ...
      _triangleMergeSortSplit(
          transformedPositions, culledIndices, tmpCulledIndices, 0, culledIndices.length ~/ 3);
    }
    ...
    for (int i = 0; i < colors.length; ++i) {
      normalTransform.transform(vec32.Vector3(
        _mesh.normals[i * 3 + 0],
        _mesh.normals[i * 3 + 1],
        _mesh.normals[i * 3 + 2],
      ).normalized());

      const b = 1.0; //xn.dot(vec32.Vector3(0.5, 0.5, 1.0).normalized()).clamp(0.1, 1.0);
...
void _triangleMergeSortSplit(
    List<vec32.Vector4> positions, List<int> dst, List<int> src, int begin, int end) {
